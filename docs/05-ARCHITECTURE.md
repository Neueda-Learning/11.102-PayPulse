...
| **Rate Limiting** *(new, MEM-020)* | `RateLimitFilter` (a `jakarta.servlet.Filter`, runs before Spring MVC dispatch) using Bucket4j + bucket4j-redis token buckets, Redis-backed for cross-instance correctness: one global system-wide bucket sized for ~40,000 req/min, plus one per-client (IP) bucket for fair-share protection. Breach → `429` + `RATE_LIMIT_EXCEEDED` + `Retry-After`/`X-RateLimit-*` headers, without ever reaching the controller/service/DB layers (cheap to reject). |
