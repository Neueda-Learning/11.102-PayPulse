...
- We considered a **distributed rate limiter (Redis-backed)** from day one for the 40k req/min target — decided to start with an **in-memory Bucket4j** bucket for Sprint 1 (single-instance demo doesn't need cross-instance coordination) and documented the Redis-backed upgrade path for when/if we run multiple instances (MEM-020/A-8) — avoids adding Redis as an infra dependency under a 7-day deadline unless truly necessary.
